 ...
 *
 * @description Harmoni_ProjectAcceptanceTrigger
 *
 * @author <A HREF="mailto:vsaran@sitetracker.com">Vartika Saran</A>
 *
***************************************************************************************************/
trigger Harmoni_ProjectAcceptanceTrigger on Project_Acceptance__c (after delete, after insert, after undelete, after update, before delete, before insert, before update) {
	try {
		strk.StTriggerFactory.executeTrigger(Harmoni_ProjectAcceptanceHandler.class, true);
	} catch (Exception ex) {
		Harmoni_Utility.insertErrorReport(
			UserInfo.getUserId(),
			ex.getMessage(),
			ex.getStackTraceString(),
			'Harmoni_ProjectAcceptanceTrigger',
			'Critical',
			true
		);
	}
